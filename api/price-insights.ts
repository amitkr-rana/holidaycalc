...
export default async function handler(
  ...
  const { departure_id, arrival_id, outbound_date, return_date, currency = 'USD' } = req.body;
  ...
  try {
    ...
    // Extract price insights from the response
    ...
    return res.status(500).json({
      error: 'Failed to fetch price insights',
